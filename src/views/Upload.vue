...
.upload
              ...
              el-upload(
                ...
                i.el-icon-upload
                  .el-upload__text 将文件拖到此处，或
                    em 点击上传
                //- el-button(size="small", icon="el-icon-upload") 点击上传
  ...
  el-dialog(
    ...
    .complete(v-else)
      p {{overContent}}
      el-button(type="primary", @click="progressVisable=false",size="small") 继续
</template>
    ...
    submitForm() {
      this.$refs["uploadForm"].validate((valid) => {
        ...
        this.$http.post('/book/uploadbook', form).then(
          res => {
            if(res) {
              this.$message.success('上传成功，感谢您的支持，奖励将于审核通过之后下发至您的账户')
...
.upload {
  ...
  .bg {
    background: url('../assets/svg/upload.svg') no-repeat 100% 100%;
    width: auto;
